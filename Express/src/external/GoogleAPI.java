...
public class GoogleAPI {
	...
	// Method 2: Calculate time for spherical distance of two locations based on
	...
	// method 3: calculate road distance of two location based on latitude and
	// longitude
	public static double road_time(Location Start, Location Dest) {

		try {
			String origins = "&origins=" + Double.toString(Start.getLatitude()) + ","
					+ Double.toString(Start.getLongitude());
			String destinations = "&destinations=" + Double.toString(Dest.getLatitude()) + ","
	...
	public static double price(double durationTime, String itemSize) {
		double size = 0;
		int selectNum = -1;
		double finalPrice;
		if (itemSize.equals("small")) {
			selectNum = 0;
		} else if (itemSize.equals("middle")) {
			selectNum = 1;
		...
		finalPrice = durationTime + size * 0.1;
		return finalPrice;
	}

	// method 6 : calculate current location for tracking
	public static Location curLocOnMap(Location start, Location end, Long stTime, Long endTime, Long curTime,
			String type) {
